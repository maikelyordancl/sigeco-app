...
const PaginationControls = ({
  ...
  <div className="flex flex-col gap-2 py-4 md:flex-row md:items-center md:justify-between">
    ...
    <div className="flex flex-wrap items-center gap-3 md:justify-end">
      {resumenLateral ? (
        <span className="text-sm font-medium text-emerald-700">{resumenLateral}</span>
      ) : null}
...
const COLUMNAS_OCULTAS_SIEMPRE = new Set(['monto_pagado_manual', 'estado_transaccion', 'monto_pagado_actual']);
const COLUMNAS_PAGO_AL_FINAL = ['estado_pago', 'monto', 'tipo_entrada'];

const getColumnLabel = (key: string, campoEtiquetaMap?: Map<string, string>) => {
  ...
  if (etiquetasFijas[key]) {
    return etiquetasFijas[key];
  }

  const etiquetaFormulario = campoEtiquetaMap?.get(key);
  if (etiquetaFormulario) {
    return etiquetaFormulario.replace(/\bmonto\b/gi, 'Valor');
  }

  return key
    .replace(/_/g, ' ')
    .replace(/\bmonto\b/gi, 'Valor')
...
export function AsistentesTable({
  ...
  const dataConNumeroFila = useMemo(() => {
    return (Array.isArray(dataState) ? dataState : []).map((row) => ({
      ...row,
    ...
    if (dataState.length > 0 && !isInitialVisibilitySet) {
      ...
      if (savedVisibility) {
        ...
        todasLasClaves.forEach((key) => {
          defaultState[key] = colsVisiblesDefault.includes(key);
        });
  ...
  const columns = useMemo<ColumnDef<Asistente>[]>(() => {
    ...
    const colsPrefijo: ColumnDef<Asistente>[] = [
      { accessorKey: 'numero_fila', header: '#', enableHiding: false },
      {
        ...
        cell: ({ row }) => (
          <div className="flex gap-2">
            <Button size="icon" className="bg-amber-500 hover:bg-amber-600" onClick={() => onEdit(row.original)}>
      ...
      },
      {
        ...
        header: 'Nombre',
        enableHiding: false,
        cell: ({ row }) => <span className="uppercase">{row.original.nombre}</span>,
      },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex justify-center">
            ...
            >
              <SelectTrigger className={`h-8 w-[200px] ${estilos.bg} ${estilos.text} border ${estilos.border}`}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {ESTADOS.map((e) => (
    ...
    const clavesVisibles = Array.from(todasLasClaves).filter(
      (key) => !clavesFijas.includes(key) && key !== '#' && !COLUMNAS_OCULTAS_SIEMPRE.has(key)
    );

    const clavesOrdenadas = [
      ...clavesVisibles.filter((key) => !COLUMNAS_PAGO_AL_FINAL.includes(key)),
      ...COLUMNAS_PAGO_AL_FINAL.filter((key) => clavesVisibles.includes(key)),
    ];

    const colsDinamicas: ColumnDef<Asistente>[] = clavesOrdenadas.map((key) => ({
      accessorKey: key,
      header: getColumnLabel(key, campoEtiquetaMap),
      cell: ({ row }) => {
  ...
  }, [dataState, onEdit, camposFormulario]);
  ...
  const paginationProps = {
    ...
    limit,
    onPageChange,
    resumenLateral: resumenPagoVisible,
  };
        ...
        <div className="md:justify-self-end flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={String(limit)} onValueChange={(v) => onLimitChange(parseInt(v, 10))}>
              <SelectTrigger className="w-[120px] border-2 border-cyan-500 shadow-md">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
