...
export default function TesoreriaPage() {
  ...
  useEffect(() => {
    const fetchEventosTesoreria = async () => {
      try {
        const response = await apiFetch("/campanas/tesoreria/campanas");
  ...
  if (loading) {
    return (
      <MainLayout title="Tesorería">
        <div className="text-center p-10">Cargando campañas...</div>
      </MainLayout>
    );
  }
      ...
      <div className="p-4 md:p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-500">
            Selecciona un evento y luego la campaña que deseas gestionar.
          </p>
        </div>
        ...
        {eventos.length > 0 ? (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {campana.subevento_nombre}
                          </p>
                        </div>

                        <Button
                          onClick={() =>
                            router.push(`/tesoreria/${campana.id_campana}`)
                          }
                        >
          ...
          <div className="text-center py-10 border-dashed border-2 rounded-lg mt-4">
            <p className="text-gray-500">
              No hay campañas activas con pago obligatorio para tesorería en este
              momento.
            </p>
          </div>
        )}
