...
exports.generarPlantillaImportacion = async (req, res) => {
    ...
    try {
        ...
        const Formulario = require('../models/formularioModel'); // Importamos si no estaba
        const camposFormulario = await Formulario.getCamposByCampanaId(id_campana);

        if (!camposFormulario || camposFormulario.length === 0) {
            return res.status(404).json({ message: 'No se encontró configuración de formulario para esta campaña.' });
        }
        ...
        // --- MAGIA DE TESORERÍA ---
        // Verificamos si la campaña requiere pago consultando a la BD
        const pool = require('../config/db');
        ...
        // Si tiene pago, añadimos dinámicamente las dos columnas opcionales
        if (tienePago) {
            columns.push({
